...
class CandleSeries
  ...
  def rsi(period = 14)
    ...
    elsif defined?(TechnicalAnalysis)
      rsi_result = TechnicalAnalysis::Rsi.calculate(closes.map { |c| { date_time: Time.current, value: c } }, period: period)
      rsi_result&.last&.value
    else
      Rails.logger.warn('[CandleSeries] RSI calculation requires ruby-technical-analysis or technical-analysis gem')
      nil
    end
  rescue ArgumentError, TypeError => e
  ...
  def moving_average(period = 20)
    return nil if candles.empty?
    return nil unless defined?(RubyTechnicalAnalysis)
  ...
  def macd(fast_period = 12, slow_period = 26, signal_period = 9)
    ...
    if defined?(RubyTechnicalAnalysis)
      ...
      macd_result = TechnicalAnalysis::Macd.calculate(closes.map { |c| { date_time: Time.current, value: c } },
                                                      fast_period: fast_period, slow_period: slow_period, signal_period: signal_period)
      return nil unless macd_result&.last

      [macd_result.last.macd, macd_result.last.signal, macd_result.last.histogram]
    else
      Rails.logger.warn('[CandleSeries] MACD calculation requires ruby-technical-analysis or technical-analysis gem')
      nil
    end
  ...
  def supertrend_signal(period: 7, multiplier: 3.0)
    return nil unless defined?(Indicators) && defined?(Indicators::Supertrend)

    result = Indicators::Supertrend.new(series: self, period: period, base_multiplier: multiplier).call
    ...
    Rails.logger.warn("[CandleSeries] Supertrend calculation failed: #{e.message}")
    nil
  end
  ...
  def bollinger_bands(period: 20, std_dev: 2.0) # rubocop:disable Lint/UnusedMethodArgument
    ...
    if defined?(RubyTechnicalAnalysis)
      ...
      bb_result = TechnicalAnalysis::BollingerBands.calculate(closes.map { |c| { date_time: Time.current, value: c } },
                                                               period: period, standard_deviation: std_dev)
      return nil unless bb_result&.last

      { upper: bb_result.last.upper_band, lower: bb_result.last.lower_band, middle: bb_result.last.middle_band }
    else
      Rails.logger.warn('[CandleSeries] Bollinger Bands calculation requires ruby-technical-analysis or technical-analysis gem')
      nil
    end
  rescue StandardError => e
    Rails.logger.warn("[CandleSeries] Bollinger Bands calculation failed: #{e.message}")
    nil
  end
  ...
  def donchian_channel(period: 20)
    ...
    Rails.logger.warn("[CandleSeries] Donchian Channel calculation failed: #{e.message}")
    nil
  end
