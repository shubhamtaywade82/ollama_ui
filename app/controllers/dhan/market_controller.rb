...
class Dhan::MarketController < ApplicationController
  ...
  def search_instruments
    ...
    if q.blank?
      return render json: { error: "q parameter required" }, status: :unprocessable_entity
    end
  ...
  def quote
    ...
    if security_id.blank? && symbol.present?
      ...
      unless security_id
        return render json: { error: "Symbol #{symbol} not found" }, status: :not_found
      end
    end

    if security_id.blank?
      return render json: { error: "security_id or symbol required" }, status: :unprocessable_entity
    end
  ...
  def option_chain
    ...
    if expiry.blank?
      return render json: { error: "expiry required (YYYY-MM-DD)" }, status: :unprocessable_entity
    end
    ...
    if underlying_security_id.blank?
      return render json: { error: "underlying_security_id or underlying_symbol required" }, status: :unprocessable_entity
    end
