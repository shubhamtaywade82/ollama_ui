...
begin
  ...
  Rails.logger.warn('[HolyGrail] ruby-technical-analysis gem not available')
end

begin
  require 'technical_analysis'
rescue LoadError
  Rails.logger.warn('[HolyGrail] technical-analysis gem not available')
end

module Indicators
  class HolyGrail < ApplicationService
    RTA = RubyTechnicalAnalysis if defined?(RubyTechnicalAnalysis)
    TA  = TechnicalAnalysis if defined?(TechnicalAnalysis)
    ...
    def initialize(candles:, config: {})
      # Accept either CandleSeries or raw hash format
      if candles.is_a?(CandleSeries)
    ...
    def call
      ...
      latest_time = Time.zone.at(stamps.last) if stamps.any?
    ...
    def lows
      if @candle_series
        ...
        @candles['low'].map(&:to_f)
      end
    end

    def stamps
      if @candle_series
        @candle_series.candles.map(&:timestamp)
      else
        @candles['timestamp'] || []
      end
    end
    ...
    def sma(len)
      closes.last(len).sum / len.to_f
    end

    def ema(len)
      return nil unless defined?(RTA) && RTA
    ...
    def macd_hash
      return { macd: 0, signal: 0, hist: 0 } unless defined?(RTA) && RTA
      ...
      Rails.logger.warn("[HolyGrail] MACD calculation failed: #{e.message}")
      { macd: 0, signal: 0, hist: 0 }
    end

    # — technical_analysis gem —
    def atr(len)
      return nil unless defined?(TA) && TA

      TA::Atr.calculate(ohlc_rows.last(len * 2), period: len).first.atr
    rescue StandardError => e
      Rails.logger.warn("[HolyGrail] ATR calculation failed: #{e.message}")
      nil
    end
    ...
    def adx(len)
      ...
      Rails.logger.warn("[HolyGrail] ADX calculation failed: #{e.message}")
      nil
    end
  end
end
