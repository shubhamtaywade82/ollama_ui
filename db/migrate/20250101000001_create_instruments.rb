...
class CreateInstruments < ActiveRecord::Migration[8.0]
  def change
    ...
    unless index_exists?(:instruments, %i[security_id symbol_name exchange segment])
      add_index :instruments, %i[security_id symbol_name exchange segment], unique: true, name: 'index_instruments_unique'
    end
    add_index :instruments, :instrument_code unless index_exists?(:instruments, :instrument_code)
    add_index :instruments, :symbol_name unless index_exists?(:instruments, :symbol_name)
    unless index_exists?(:instruments, %i[underlying_symbol expiry_date])
